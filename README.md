...
3. Скрипт сам скачает Node.js в папку проекта (система не трогается, права
   администратора не нужны), поставит зависимости, соберёт сайт и откроет его
   в браузере. Первый запуск — 3–6 минут (нужен интернет), дальше — мгновенно.

Ничего заранее устанавливать не нужно. Если macOS спросит подтверждение —
правый клик по `start.command` → «Открыть», либо в Терминале: `bash start.command`.
