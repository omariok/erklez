...
## Показать сайт на MacBook (одна кнопка)

1. Передать папку проекта (например, zip-архивом) — `node_modules` и `.next` не нужны.
2. На Mac распаковать и **двойным кликом открыть `start.command`**.
3. Скрипт сам поставит зависимости, соберёт сайт и откроет его в браузере
   (первый запуск — 2–4 минуты, дальше — мгновенно).

Требование: Node.js 18+ (https://nodejs.org). Скрипт проверяет это сам:
если Node нет, а Homebrew установлен — поставит Node сам; иначе подскажет ссылку.
Если macOS спросит подтверждение — правый клик по `start.command` → «Открыть»,
либо в Терминале: `bash start.command`.

Без заполненного `.env` заявки не уходят в Telegram (форма покажет ошибку) —
для просмотра сайта это не важно.
...
Слой данных: `lib/content-source.ts` (типизированные async-геттеры с фолбэком),
схемы: `sanity/schemaTypes/`. Секции главной уже получают данные пропсами через
`content-source`, поэтому после заполнения CMS контент подхватится без правок кода.
